class Friends
  ...
  def making_friends_sheet(package:)
    package.workbook.add_worksheet(name: 'Making friends') do |sheet|
      ...
      @grouped_by[:weekend].each do |type_of_day, count|
      ...
      @grouped_by[:week_year].sort_by { |_week_year, count| count }.reverse.each do |week_year, count|
      ...
      @grouped_by[:month_year].sort_by { |_month_year, count| count }.reverse.each do |month_year, count|
        sheet.add_row [month_year, count]
      end

      sheet.add_row ['Most busy making friends days']
      sheet.add_row ['Day', 'Number of friends added']
      @grouped_by[:day].sort_by { |_day, count| count }.reverse.each do |day, count|
        sheet.add_row [day, count]
      end
    end
  end
